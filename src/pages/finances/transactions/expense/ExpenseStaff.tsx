...
export const ExpenseStaff: React.FC = () => {
  ...
  const transactionItems = TransactionsService.GetTransactionItemsList({
    assign_types: assign_types,
  });

  useEffect(() => {
    if (action === "edit" && id !== undefined) {
      TransactionsService.GetTransactionById({ id: id }, "expense").then(
        (res: any) => {
          setForm({
            ...form,
            values: {
  ...
  return (
    <section>
      ...
      <h1 className="text-[32px] font-bold">
        {action === "add" && "Добавление расхода (сотрудник)"}
        {action === "edit" && "Редактирование расхода (сотрудник)"}
      </h1>

      <form onSubmit={handleFormSubmit} className="w-full mt-[60px] pb-[60px]">
        <div className="w-full px-[37px] py-[40px] bg-white rounded-[7px] grid grid-cols-3 gap-[50px]">
          <FormControl required variant="standard">
            ...
            <FormHelperText>
              {form.validation.message.box_office}
            </FormHelperText>
          </FormControl>
          ...
          <FormControl required variant="standard">
            ...
            <Select
              ...
              error={form.validation.error.staff}
              onChange={(event: any) => {
                setForm({
                  ...form,
                  values: {
            ...
            <FormHelperText>{form.validation.message.staff}</FormHelperText>
          </FormControl>
        ...
        <div className="w-full mt-[40px] bg-white shadow-block rounded-[7px] py-[40px] px-[37px] gap-[50px]">
          ...
          {form.values.expense.map((expense: any, index: any) => (
            <div
              key={index}
              className="w-full flex items-end gap-[26px] mb-[40px]"
            >
                ...
                >
                  ...
                  >
                    {!transactionItems.loading &&
                      ...
                      transactionItems.result?.data.map(
